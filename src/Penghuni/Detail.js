...
import { RefreshControl,StyleSheet,ScrollView, Text, View,TouchableOpacity,Modal,Image,TextInput,ActivityIndicator,Alert,Share } from 'react-native'
...
right:50,
padding:5 }}
     onPress={() => handleDownload(datagambar)}>

<Text numberOfLines={2}
style={{ color: '#fff',
    fontWeight: '800',
    fontSize: 12}}>Download
</Text>
...
{Array.isArray(foto)
  ...
  const handleDownload = (datagambar) => {
    // Dapatkan direktori penyimpanan untuk gambar yang akan diunduh
    const dirs = RNFetchBlob.fs.dirs;
    const fileName = 'gambar_unduhan.jpg'; // Nama file yang diinginkan
    ...
    RNFetchBlob
      ...
      .fetch('GET', host+"cfm_ug/Doc/incident/"+datagambar)
      .then(res => {
        // Unduhan berhasil
        Alert.alert('Sukses', 'Gambar berhasil diunduh.');
      })
      .catch(error => {
        // Handle kesalahan
        console.log(error);
        Alert.alert('Error', 'Gagal mengunduh gambar.');
      });
  };
