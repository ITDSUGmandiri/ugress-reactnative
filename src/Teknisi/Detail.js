...
const { incident_id } = route.params;
  ...
  const [fullscreenVisibleincident, setFullscreenVisibleincident] = useState(false);
  ...
  const handleImagePress = (img) => {
   ...
   const handleImagePressincident = (img) => {
    setGambar(img);
    setFullscreenVisibleincident(true);
  };

    const handleCloseFullscreen = () => {
    setFullscreenVisible(false);
  };
     const handleCloseFullscreenincident = () => {
    setFullscreenVisibleincident(false);
  };
...
const TimelineEvent = ({ date, event, id,username }) => (
  <View style={{ flexDirection: 'row', marginBottom: 0 }}>
   ...
   <View  style={{ width: 20, height: 20 ,backgroundColor: '#56c0e0' ,borderRadius:30,top:0,position:'absolute'}} ></View>
    ...
    <View style={{ flex: 1 }}>
      <Text style={{ fontWeight: 'bold', marginTop: 5 ,color:'#000060'}}>{date}</Text>
      <Text style={{color:'#000060'}}>{username}</Text>
      <Text style={{color:'#000060'}}>{event}</Text>
    </View>
  </View>
);
...
return (
 <View style={styles.Card1}>
   <Modal animationType="none" visible={fullscreenVisibleincident}>
<View 
    style={{  flex: 1,
  ...
  <Image source={{uri:host+"cfm_ug/Doc/incident/"+datagambar}} style={styles.image}></Image>
...
right:10,
padding:5 }}
 onPress={handleCloseFullscreenincident}>
...
textAlign:'right',
padding:5 }}
onPress={() =>  navigation.navigate('Teknisi_tambah_foto',{incident_id:incident_id})}>

<Text numberOfLines={2}
style={{ color: '#fff',
    fontWeight: '800',
    fontSize: 14}}>Tambah Foto
</Text>
...
{Array.isArray(foto)
       ...
       <TouchableOpacity  onPress={() => handleImagePressincident(us.foto)}>
...
textAlign:'right',
padding:5 }}
onPress={() => Pindah_halaman('Ttd')}>

<Text numberOfLines={2}
style={{ color: '#fff',
    fontWeight: '800',
    fontSize: 14}}>Tambah sign
</Text>
...
{Array.isArray(data)
        ? data.map((us,i) => {
          return ( 
          <View key={i}>
          <TouchableOpacity  onPress={() => handleImagePress(us.file_upload)}>
...
<View 
    ...
    <Image source={{uri:host+"cfm_ug/Doc/sign/"+datagambar}} style={styles.image}></Image>
...
navigation.replace(halaman,{incident_id:incident_id});
...
style={{width: lebar,fontSize:18,color:'#000060',fontWeight:'bold',marginLeft:10,top:5}}>Biaya Perbaikan : 
    ...
    <View style={{ flex: 1 }}>   
   <Text numberOfLines={4}
  style={{width: lebar,fontSize:18,color:'#000060',fontWeight:'bold',marginLeft:10,top:5}}>Sign : 
  </Text>
      <Ttd />
    </View>
...
<TouchableOpacity style={{ width:'45%',
        ...
        paddingVertical: 13,
        elevation: 8,}} 
      onPress={() => navigation.navigate('Teknisi_pending',{incident_id:incident_id})}
       activeOpacity={0.6}>
...
<TouchableOpacity style={{ width:'45%',
        ...
        paddingVertical: 13,
        elevation: 8,}} 
      onPress={() => navigation.replace('Teknisi_finish',{incident_id:incident_id})}
       activeOpacity={0.6}>
